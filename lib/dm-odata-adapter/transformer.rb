    ...
    module Odata
      module Transformer
        ...
        def transform_dm_resource_to_odata_remote_class(the_resource, class_name)
          ...
          @log.debug("resource_to_remote(#{class_name}, #{hash})")
          ...
          @log.debug("Returning instance #{instance.inspect}")
          instance
        end
        ...
        def collection_from_remote(model, array)
          @log.debug("collection_from_remote is about to parse\n #{array.inspect}")
        ...
        def register_model(model, class_name)
          @log.debug("register_model(#{model}, #{class_name})")
          the_fields = model.properties.map { |p| p.field }
          @log.debug("the_fields are #{the_fields.inspect}")
          return nil
          if(@class_registry.has_key?(model))
            clazz = @class_registry.fetch(model)
            @log.debug("FOUND #{model} in registry as #{clazz.new.inspect}")
          else
            @class_registry[model] = create_odata_class(class_name, the_fields)
            @log.debug("REGISTERED #{model} #{@class_registry[class_name]}")
          end
        end
        ...
        def create_odata_class(class_name, the_fields)
          ...
          @log.debug("Created new class #{class_name} as #{klass.new.inspect} with #{the_fields.inspect}")
          klass
        end
        
        def object_from_remote(model, remote_instance)
          @log.debug("object_from_remote is about to parse\n #{remote_instance.inspect}")
          field_to_property = make_field_to_property_hash(model)
          record_from_remote(remote_instance, field_to_property)
        end
        ...
        def record_from_remote(instance, field_to_property)
          @log.debug("record_from_remote using:\n#{field_to_property}\nAnd #{instance.inspect}")
          record = {}
          ...
          @log.debug("record_from_remote returning #{record}")
          record
        end
